...
halvan = [halvan333, halvan458, halvan555, halvan888, halvan101010]
halvanx = [halvanx555]

machine = "halvan_"
gbasedir = '/Users/jonatanlinden/Documents/dedup_meas/halvan/memcont/'
...
def chunk_oh_vs_rate(conf):
    ...
    lt_anchor_l = interval_analysis(lt_anchor, anchor_cls, anchor_l, timechunk=1000000000)
    lt_chunk_only_l = interval_analysis(lt_chunk_only, chunk_cls, chunk_l, timechunk=1000000000)
    ...
    megatuple = zip(anchor_ths, chunk_ths, compress_ths, anchor_rate, chunk_rate, compress_rate, avg_mem_oh, no_items_in_q)
    return megatuple


if __name__ == '__main__':
    megatuples = map (chunk_oh_vs_rate, halvanx)
    for m,h in zip(megatuples, halvanx):
        with open(gbasedir + h.relpath + "data/" + 'thru_vs_oh_' + machine + ".".join(map(str,h.dist)) + '.dat', 'w') as f:
